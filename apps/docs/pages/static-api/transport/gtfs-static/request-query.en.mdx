...
For example, to retrieve information about transit agencies, you can use the following query:
...
- This query will provide a download link for the GTFS data in its original .txt format.
...
- By default, or when `download` is set to false, the data will be returned in JSON format.
