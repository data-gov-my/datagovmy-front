...
API Cuaca menyediakan maklumat cuaca secara waktu sebenar, termasuk ramalan umum 7 hari dan amaran cuaca secara langsung. Tambahan pula, amaran gempa bumi disediakan dalam 'endpoint' yang berasingan disebabkan format yang unik. Data disediakan oleh MET Malaysia (Jabatan Meteorologi Malaysia). Untuk maklumat cuaca yang lebih komprehensif dan sumber tambahan, anda boleh mengunjungi [Portal Rasmi MET Malaysia](https://www.met.gov.my/).

_Catatan: Data ramalan laut kini tidak tersedia._
...
### Pertanyaan & Format Respons

Untuk menyaring dan meminta data, gunakan operasi penyaringan standard dalam [Permintaan Pertanyaan](/request-query). Untuk maklumat tentang bagaimana API ini memberi respons kepada permintaan anda, rujuk bahagian [Format Tindak Balas](/response-format).
...
Untuk mendapatkan data dari API Cuaca, gunakan arahan `curl` berikut. Buka terminal anda dan masukkan arahan ini:

```curl
curl --location 'https://api.data.gov.my/weather/forecast?limit=3'
```

#### Penyaringan Bidang-Bidang Bersarang

```json
    {
        // 'double underscore' digunakan
        "location": {
...
Data respons berstruktur seperti di atas. Anda boleh mengakses bidang-bidang bersarang menggunakan **'double underscore'**. Sebagai contoh, untuk menyaring data ramalan untuk Langkawi, anda boleh menggunakan parameter pertanyaan `?contains=Langkawi@location__location_name`.
...
`location_id` berada dalam format "AA000," di mana awalan abjad mewakili jenis lokasi. Terdapat lima jenis lokasi yang mungkin:
...
Untuk menyaring data ramalan untuk kategori lokasi tertentu, cukup gunakan parameter pertanyaan `contains`. Sebagai contoh, untuk mendapatkan data ramalan lokasi Negeri sahaja, gunakan `?contains=St@location__location_id`.

### Memahami Nilai Bidang-Bidang

| Key                  | Jenis   | Penerangan                                                                                                                              | Contoh Nilai                                                                                                                                               |
| -------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `location_id`        | string  | Pengenal unik untuk lokasi. Lihat [Penyaringan mengikut kategori lokasi](#penyaringan-mengikut-kategori-lokasi) untuk lebih keterangan. | <ul> <li>`"St001"` (Negeri)</li> <li>`"Rc001"` (Rekreasi)</li> <li>`"Ds001"` (Daerah)</li> <li>`"Tn001"` (Bandar)</li> <li>`"Dv001"` (Bahagian)</li> </ul> |
...
| `summary_forecast`   | string  | Ringkasan ramalan hari.                                                                                                                 | `"Ribut petir di satu dua tempat"`                                                                                                                         |
...
#### Nilai-nilai untuk Bidang `*_forecast`

Nilai-nilai untuk `morning_forecast`, `afternoon_forecast`, `night_forecast`, dan `summary_forecast`:
...
#### Nilai Mungkin untuk Bidang `summary_when`
...
Data respons berstruktur seperti di atas. Anda boleh mengakses bidang-bidang bersarang menggunakan **'double underscore'**. Sebagai contoh, untuk menyaring amaran yang dikeluarkan bermula dari 1957-08-31 ke atas, anda boleh menggunakan parameter pertanyaan `?timestamp_start=1957-08-31 00:00:00@warning_issue__issued`.
...
Data gempa bumi disediakan dalam 'endpoint' yang didedikasikan daripada `weather/warning` disebabkan format yang unik.
...
| Key                 | Jenis    | Penerangan                                          | Contoh Nilai                      |
| ------------------- | -------- | --------------------------------------------------- | --------------------------------- |
| `utcdatetime`       | datetime | Tarikh dan masa UTC gempa bumi.                     | "2023-10-09T03:50:50"             |
| `localdatetime`     | datetime | Tarikh dan masa tempatan (UTC+08:00) gempa bumi.    | "2023-10-09T11:50:50"             |
...
| `magdefault`        | float    | Magnitud lalai gempa bumi.                          | 5.5                               |
| `magtypedefault`    | string   | Jenis magnitud lalai gempa bumi.                    | "mb"                              |
