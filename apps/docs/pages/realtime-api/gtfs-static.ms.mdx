...
API GTFS Static menyediakan akses kepada jadual pengangkutan awam serta maklumat geospatial yang telah dipiawaikan mengikuti General Transit Feed Specification (GTFS). Untuk mendapat maklum lanjut mengenai GTFS, anda boleh merujuk [dokumentasi GTFS](https://developers.google.com/transit/gtfs) yang disediakan oleh Google.
...
Fail lain seperti `frequencies.txt` dan `shapes.txt` dibekalkan sekiranya tersedia oleh agensi. Untuk butiran lanjut, anda boleh merujuk rujuk [Rujukan Transit Statik](https://gtfs.org/schedule/reference/).
